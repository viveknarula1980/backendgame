  ...
  io.on("connection", (socket) => {
    ...
    socket.on("coinflip:join", async ({ player, side, entryLamports, clientSeed }) => {
      try {
        ...
        // If player explicitly wants bot, skip queue and match instantly
if (arguments[0].playWithBot) {
  const botSide = 1 - s;
  const A = { socketId: socket.id, playerPk: playerPk.toBase58(), entryLamports: String(stake), side: s, clientSeed: String(clientSeed || "") };
  const B = { socketId: null, playerPk: (await getServerKeypair()).publicKey.toBase58(), entryLamports: String(stake), side: botSide, clientSeed: "", isBot: true };
  try {
    await createRoom(io, A, B);
  } catch (e) {
    return socket.emit("coinflip:error", { code: "BOT_FAIL", message: String(e.message || e) });
  }
  return;
}
